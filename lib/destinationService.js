...
export async function getDynamicDestinationData(countryName) {
    ...
    // Generate potential slugs for lookup
    const slug = countryName.toLowerCase().replace(/ /g, '-').replace(/[^\w-]/g, '');
    const cleanSlug = countryName.toLowerCase().replace(/[^a-z0-9]/g, '');

    console.log(`[CountryService] 🏛️ Reading single-source-of-truth: "${slug}"`);

    // Load the consolidated editorial file
    let allEditorial;
    try {
        const editorialContent = await fs.readFile(EDITORIAL_PATH, "utf-8");
    ...
    // Lookup strategy: Try hyphenated slug first, then clean slug
    let ed = allEditorial[slug] || allEditorial[cleanSlug];

    // If still not found, try a fuzzy match (case-insensitive keys)
    if (!ed) {
        const keys = Object.keys(allEditorial);
        const fuzzyKey = keys.find(k => k.replace(/-/g, '') === cleanSlug);
        if (fuzzyKey) ed = allEditorial[fuzzyKey];
    }

    if (!ed) {
        console.error(`[CRITICAL] Country "${countryName}" not found in countries-editorial.json (tried: ${slug}, ${cleanSlug})`);
        throw new Error(`Content Error: Country "${countryName}" is not supported in the static database.`);
    }
