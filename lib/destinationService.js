...
export async function getDynamicDestinationData(countryName) {
    ...
    if (!ed) {
        console.error(`[CountryService] CRITICAL: Country "${countryName}" not found in any source.`);
        throw new Error(`Content Error: Country "${countryName}" not found.`);
    }

    // Map best places from the new structure (popular + underrated)
    let bestPlacesMapping = [];
    if (ed.placesToVisit) {
        if (Array.isArray(ed.placesToVisit)) {
            bestPlacesMapping = ed.placesToVisit.map(p => ({
                city: p.name,
            ...
            const pop = (ed.placesToVisit.popular || []).map(p => ({
                city: p.name,
                slug: p.slug,
                reason: "Popular Destination",
                category: "Popular"
            }));
            const und = (ed.placesToVisit.underrated || []).map(p => ({
                city: p.name,
                slug: p.slug,
                reason: "Hidden Gem",
                category: "Underrated"
            }));
            bestPlacesMapping = [...pop, ...und];
        }
    }
    ...
    const data = {
        ...
        content: {
            ...
            thingsToDo: bestPlacesMapping.map(p => p.city),
            blogPosts: [],
            sections: {
